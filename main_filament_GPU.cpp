...
int o = 5000001, writeInterval=491;
...
int main()
{
    ...
    float x_lower_left = -12.50, x_lower_right = 30.0, y_lower_left = -12.50, y_upper_left = 12.50;
    float Hx2 = 1.5, Hx1 = -0.5, Hy2 = 1.5, Hy1 = -1.5;
    int m_div1/* = 15*/, m_div2 = 400, m_div3/* = 75*/, m, n_div1/* = 5*/, n_div2 = 600, n_div3/* = 15*/, n,m_u,n_u,m_v,n_v,c5,probe_i,probe_j;
    int m_div11, m_div22, n_div11, n_div22, c11, c22, c33, c44;
    //float radius_a = 0.5, radius_b = radius_a/6.0;
    int nn=10;   // Number of pannels in between two nodes for interpolation
    int N_markerf = Nf+(Nf+2), N_marker=N_markerf*nn; //Please take a number divisible by 4
    int ctr1, ctr2;
    float rate_xu = 1.1, rate_xd = 1.01, rate_yu = 1.02, rate_yd = 1.02;
    ...
    double Lf=1.0;
    double kappa=0.0*pi, gamma=1.5, Fr=0.0, g_xg=0.0, g_yg=0.0, beta=1.0;
    ...
    double x_sfc[N_markerf],y_sfc[N_markerf],thetafc[Nt], minfx1, minfx2, maxfx1, maxfx2, minfy1, minfy2, maxfy1, maxfy2;
    double Afc1,Afc2,Afc3,Afc4,Bfc1,Bfc2,Bfc3,Bfc4,Cfc1,Cfc2,Cfc3,Cfc4;
    double amplitude_f=0.575, theta_amplitude=0.0, omega_f=4.0, phi_pitching=0.0, phi_heaving=0.0, theta_pitching, beta_f=pi/2.0;
    double yf_c,xf_c;
    //cout<<fixed <<setprecision(20)<<ds<<endl;
    xf_c = amplitude_f*sin(omega_f*(time1))*cos(beta_f);
    yf_c = amplitude_f*sin(omega_f*(time1))*sin(beta_f);
    theta_pitching = theta_amplitude*sin(omega_f*(time1)+phi_pitching);
    for (int i=0; i<Nf; i++)
    {
		xf[i]=xf_0 + i*ds*cos(kappa)+xf_c;
		yf[i]=yf_0 + i*ds*sin(kappa)+yf_c;
		xf_old2[i]=xf[i];
		yf_old2[i]=yf[i];
		xf_old1[i]=xf[i];
		yf_old1[i]=yf[i];
    ...
    radius_a=(xf_old1[Nf-1]-xf_old1[0])/2.0;
    radius_b=0.06*radius_a;
    for (int i=0; i<Nf; i++)
    {
        hf[i]=hff/2.0;
    }
...
// filament parameters end
    ...
    for (int i=0; i<Nf+1; i++)
    {
        ...
        {
			//x_sf[i]=xf_old1[i-1] - hf[i-1]*sin(thetaf[i-1]);
            //y_sf[i]=yf_old1[i-1] + hf[i-1]*cos(thetaf[i-1]);
			if ((xf_old1[i]-xf_old1[i-1])>=0.0 && thetaf[i-1]>=0.0)
			{
				x_sf[i]=xf_old1[i-1] - hf[i-1]*sin(thetaf[i-1]);
				y_sf[i]=yf_old1[i-1] + hf[i-1]*cos(thetaf[i-1]);
			}
			else if ((xf_old1[i]-xf_old1[i-1])<0.0 && thetaf[i-1]<=0.0)
			{
				x_sf[i]=xf_old1[i-1] + hf[i-1]*sin(thetaf[i-1]);
				y_sf[i]=yf_old1[i-1] - hf[i-1]*cos(thetaf[i-1]);
			}
        ...
        else
        {
            if (i==Nf)
            {
                if ((xf_old1[i-1]-xf_old1[i-2])>=0.0 && thetaf[i-1]>=0.0)
				{
				x_sf[i]=xf_old1[i-1] - hf[i-1]*sin(thetaf[i-1]);
				y_sf[i]=yf_old1[i-1] + hf[i-1]*cos(thetaf[i-1]);
				}
				else if ((xf_old1[i-1]-xf_old1[i-2])<0.0 && thetaf[i-1]<=0.0)
				{
					x_sf[i]=xf_old1[i-1] + hf[i-1]*sin(thetaf[i-1]);
					y_sf[i]=yf_old1[i-1] - hf[i-1]*cos(thetaf[i-1]);
				}
            ...
            {
                if ((xf_old1[i]-xf_old1[i-2])>=0.0 && thetaf[i-1]>=0.0)
                {
                    x_sf[i]=xf_old1[i-1] - hf[i-1]*sin(thetaf[i-1]);
                    y_sf[i]=yf_old1[i-1] + hf[i-1]*cos(thetaf[i-1]);
                }
                else if ((xf_old1[i]-xf_old1[i-2])<0.0 && thetaf[i-1]<=0.0)
                {
                    x_sf[i]=xf_old1[i-1] + hf[i-1]*sin(thetaf[i-1]);
                    y_sf[i]=yf_old1[i-1] - hf[i-1]*cos(thetaf[i-1]);
                }
    ...
    for (int i=Nf+1; i<N_markerf; i++)
    {
        ...
        else if (i==N_markerf-1)
        {
            //x_sf[i]=xf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
			//y_sf[i]=yf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
			if ((xf_old1[N_markerf-i]-xf_old1[N_markerf-i-1])>=0.0 && thetaf[N_markerf-i-1]>=0.0)
			{
				x_sf[i]=xf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
				y_sf[i]=yf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
			}
			else if ((xf_old1[N_markerf-i]-xf_old1[N_markerf-i-1])<0.0 && thetaf[N_markerf-i-1]<=0.0)
			{
				x_sf[i]=xf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
				y_sf[i]=yf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
			}
        ...
        else
        {
            if (i==Nf+2)
            {
                if ((xf_old1[N_markerf-i-1]-xf_old1[N_markerf-i-2])>=0.0 && thetaf[N_markerf-i-1]>=0.0)
				{
					x_sf[i]=xf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
					y_sf[i]=yf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				}
				else if ((xf[N_markerf-i-1]-xf[N_markerf-i-2])<0.0 && thetaf[N_markerf-i-1]<=0.0)
				{
					x_sf[i]=xf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
					y_sf[i]=yf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				}
				else if ((xf_old1[N_markerf-i-1]-xf_old1[N_markerf-i-2])>=0.0 && thetaf[N_markerf-i-1]<=0.0)
				{
					x_sf[i]=xf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
					y_sf[i]=yf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				}
            ...
            {
                if ((xf_old1[N_markerf-i]-xf_old1[N_markerf-i-2])>=0.0 && thetaf[N_markerf-i-1]>=0.0)
				{
					x_sf[i]=xf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
					y_sf[i]=yf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				}
				else if ((xf_old1[N_markerf-i]-xf_old1[N_markerf-i-2])<0.0 && thetaf[N_markerf-i-1]<=0.0)
				{
					x_sf[i]=xf_old1[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
					y_sf[i]=yf_old1[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				}
...
//Generating surface points for flagging using linear interpolation
    for (int i=0; i<N_marker; i++)
    {
		...
		else
		{
			if (ctr1==N_markerf-1)
			{
				dx = (x_sf[0]-x_sf[ctr1])/(nn);
				dy = (y_sf[0]-y_sf[ctr1])/(nn);
			}
			else
			{
				dx = (x_sf[ctr1+1]-x_sf[ctr1])/(nn);
				dy = (y_sf[ctr1+1]-y_sf[ctr1])/(nn);
			}
			...
			{
				x_s[i] = x_sf[ctr1];
				y_s[i] = y_sf[ctr1] + (i-ctr1*nn)*dy;
			}
			else
			{
				x_s[i] = x_sf[ctr1] + (i-ctr1*nn)*dx;
				if (ctr1==N_markerf-1)
				{
					y_s[i] = y_sf[ctr1] + (y_sf[0]-y_sf[ctr1])/(x_sf[0]-x_sf[ctr1])*(x_s[i]-x_sf[ctr1]);
				}
				else
				{
    ...
    ofstream file_lift3f("Lift_inst.dat");
    ofstream file_drag3f("Drag_inst.dat");
    ...
    for(int k=1;k<=o;k++)   //time marching
    {
		...
		for (int kk=1;kk<=G_Str;kk++)
		{
			...
			for (int i=1;i<Nf;i++)
			{
				if (i==1)
				{
					Fb_x[i]=(-gamma/pow(ds,4))*(x_star[i+2]-4.0*x_star[i+1]+7.0*x_star[i]-4.0*x_star[i-1]-2.0*ds*sin(theta_pitching+beta_f));
					Fb_y[i]=(-gamma/pow(ds,4))*(y_star[i+2]-4.0*y_star[i+1]+7.0*y_star[i]-4.0*y_star[i-1]+2.0*ds*cos(theta_pitching+beta_f));
				}
				else if (i==Nf-2)
				{
					Fb_x[i]=(-gamma/pow(ds,4))*(-2.0*x_star[i+1]+5.0*x_star[i]-4.0*x_star[i-1]+x_star[i-2]);
					Fb_y[i]=(-gamma/pow(ds,4))*(-2.0*y_star[i+1]+5.0*y_star[i]-4.0*y_star[i-1]+y_star[i-2]);
				}
				else if (i==Nf-1)
				{
					Fb_x[i]=(-2.0*gamma/pow(ds,4))*(x_star[i]-2.0*x_star[i-1]+x_star[i-2]);
					Fb_y[i]=(-2.0*gamma/pow(ds,4))*(y_star[i]-2.0*y_star[i-1]+y_star[i-2]);
				}
				else
				{
					Fb_x[i]=(-gamma/pow(ds,4))*(x_star[i+2]-4.0*x_star[i+1]+6.0*x_star[i]-4.0*x_star[i-1]+x_star[i-2]);
					Fb_y[i]=(-gamma/pow(ds,4))*(y_star[i+2]-4.0*y_star[i+1]+6.0*y_star[i]-4.0*y_star[i-1]+y_star[i-2]);
				}
			}
			for (int i=1;i<Nt-1;i++)
			{
				A_f[i]=beta*(1.0/(2.0*pow(dt1,2)))*(1.0-(1.0/pow(ds,2))*(2.0*pow((xf_old1[i]-xf_old1[i-1]),2)+2.0*pow((yf_old1[i]-yf_old1[i-1]),2)-pow((xf_old2[i]-xf_old2[i-1]),2)-pow((yf_old2[i]-yf_old2[i-1]),2)));
				B_f[i]=beta*(1.0/(pow(dt1,2)*pow(ds,2)))*(pow((xf_old1[i]-xf_old2[i]-xf_old1[i-1]+xf_old2[i-1]),2) + pow((yf_old1[i]-yf_old2[i]-yf_old1[i-1]+yf_old2[i-1]),2));
				if (i==1)
				{
					C_f[i]=((x_star[i]-x_star[i-1])*((Fb_x[i]+(drag3f[i]/(2.0*ds)))+(beta*Fr*g_xg+beta*pow(omega_f,2)*amplitude_f*sin(omega_f*(time1+kk*dt1))*cos(beta_f))) + (y_star[i]-y_star[i-1])*((Fb_y[i]+(lift3f[i]/(2.0*ds)))+(beta*Fr*g_yg+beta*pow(omega_f,2)*amplitude_f*sin(omega_f*(time1+kk*dt1))*sin(beta_f))))/(pow(ds,2));
				}
				else
				{
					C_f[i]=((x_star[i]-x_star[i-1])*((Fb_x[i]+(drag3f[i]/(2.0*ds)))-(Fb_x[i-1]+(drag3f[i-1]/(2.0*ds)))) + (y_star[i]-y_star[i-1])*((Fb_y[i]+(lift3f[i]/(2.0*ds)))-(Fb_y[i-1]+(lift3f[i-1]/(2.0*ds)))))/(pow(ds,2));
				}
				if (i==1)
				{
					A_t[i]=-(1.0/(pow(ds,4)))*(pow((x_star[i]-x_star[i-1]),2) + pow((y_star[i]-y_star[i-1]),2));
					B_t[i]=(1.0/(pow(ds,4)))*((x_star[i]-x_star[i-1])*(x_star[i+1]-x_star[i]) + (y_star[i]-y_star[i-1])*(y_star[i+1]-y_star[i]));
					C_t[i]=0.0;
				}
				else if (i==Nt-2)
				{
					A_t[i]=-(3.0/(pow(ds,4)))*(pow((x_star[i]-x_star[i-1]),2) + pow((y_star[i]-y_star[i-1]),2));
					B_t[i]=0.0;
					C_t[i]=(1.0/(pow(ds,4)))*((x_star[i]-x_star[i-1])*(x_star[i-1]-x_star[i-2]) + (y_star[i]-y_star[i-1])*(y_star[i-1]-y_star[i-2]));
				}
				else
				{
					A_t[i]=-(2.0/(pow(ds,4)))*(pow((x_star[i]-x_star[i-1]),2) + pow((y_star[i]-y_star[i-1]),2));
					B_t[i]=(1.0/(pow(ds,4)))*((x_star[i]-x_star[i-1])*(x_star[i+1]-x_star[i]) + (y_star[i]-y_star[i-1])*(y_star[i+1]-y_star[i]));
					C_t[i]=(1.0/(pow(ds,4)))*((x_star[i]-x_star[i-1])*(x_star[i-1]-x_star[i-2]) + (y_star[i]-y_star[i-1])*(y_star[i-1]-y_star[i-2]));
				}
			}
	...
	// Solving filament positions (xf and yf)
			for (int PX=1;PX<=X_SOR;PX++)
			{
				...
				for (int i=0;i<Nf;i++)
				{
					if (i==0)
					{
						xf[i]=amplitude_f*sin(omega_f*(time1+kk*dt1))*cos(beta_f);
						yf[i]=amplitude_f*sin(omega_f*(time1+kk*dt1))*sin(beta_f);
					}
					else if (i==1)
					{
						xf[i]=alpha_SOR_f*(((-gamma*pow(dt1,2)/pow(ds,4))*xf[i+2] + pow(dt1,2)*((Zeta[i+1]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*xf[i+1] + pow(dt1,2)*((Zeta[i]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*xf[i-1] + 2.0*sin(theta_pitching+beta_f)*gamma*pow(dt1,2)/pow(ds,3) + (beta*pow(dt1,2)*Fr*g_xg) + pow(dt1,2)*(drag3f[i]/(2.0*ds)) + (2.0*beta*xf_old1[i]) - (beta*xf_old2[i]))/(beta+(pow((dt1/ds),2)*(Zeta[i+1]+Zeta[i]))+(7.0*gamma*pow(dt1,2)/pow(ds,4)))) + (1.0-alpha_SOR_f)*xf[i];

						yf[i]=alpha_SOR_f*(((-gamma*pow(dt1,2)/pow(ds,4))*yf[i+2] + pow(dt1,2)*((Zeta[i+1]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*yf[i+1] + pow(dt1,2)*((Zeta[i]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*yf[i-1] - 2.0*cos(theta_pitching+beta_f)*gamma*pow(dt1,2)/pow(ds,3) + (beta*pow(dt1,2)*Fr*g_yg) + pow(dt1,2)*(lift3f[i]/(2.0*ds)) + (2.0*beta*yf_old1[i]) - (beta*yf_old2[i]))/(beta+(pow((dt1/ds),2)*(Zeta[i+1]+Zeta[i]))+(7.0*gamma*pow(dt1,2)/pow(ds,4)))) + (1.0-alpha_SOR_f)*yf[i];
					}
					else if (i==Nf-2)
					{
						xf[i]=alpha_SOR_f*((pow(dt1,2)*((Zeta[i+1]/pow(ds,2))+(2.0*gamma/pow(ds,4)))*xf[i+1] + pow(dt1,2)*((Zeta[i]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*xf[i-1] - (gamma*pow(dt1,2)/pow(ds,4))*xf[i-2] + (beta*pow(dt1,2)*Fr*g_xg) + pow(dt1,2)*(drag3f[i]/(2.0*ds)) + (2.0*beta*xf_old1[i]) - (beta*xf_old2[i]))/(beta+(pow((dt1/ds),2)*(Zeta[i+1]+Zeta[i]))+(5.0*gamma*pow(dt1,2)/pow(ds,4)))) + (1.0-alpha_SOR_f)*xf[i];

						yf[i]=alpha_SOR_f*((pow(dt1,2)*((Zeta[i+1]/pow(ds,2))+(2.0*gamma/pow(ds,4)))*yf[i+1] + pow(dt1,2)*((Zeta[i]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*yf[i-1] - (gamma*pow(dt1,2)/pow(ds,4))*yf[i-2] + (beta*pow(dt1,2)*Fr*g_yg) + pow(dt1,2)*(lift3f[i]/(2.0*ds)) + (2.0*beta*yf_old1[i]) - (beta*yf_old2[i]))/(beta+(pow((dt1/ds),2)*(Zeta[i+1]+Zeta[i]))+(5.0*gamma*pow(dt1,2)/pow(ds,4)))) + (1.0-alpha_SOR_f)*yf[i];
					}
					else if (i==Nf-1)
					{
						xf[i]=alpha_SOR_f*((pow(dt1,2)*((2.0*Zeta[i]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*xf[i-1] - (2.0*gamma*pow(dt1,2)/pow(ds,4))*xf[i-2] + (beta*pow(dt1,2)*Fr*g_xg) + pow(dt1,2)*(drag3f[i]/(2.0*ds)) + (2.0*beta*xf_old1[i]) - (beta*xf_old2[i]))/(beta+(2.0*pow((dt1/ds),2)*Zeta[i])+(2.0*gamma*pow(dt1,2)/pow(ds,4)))) + (1.0-alpha_SOR_f)*xf[i];

						yf[i]=alpha_SOR_f*((pow(dt1,2)*((2.0*Zeta[i]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*yf[i-1] - (2.0*gamma*pow(dt1,2)/pow(ds,4))*yf[i-2] + (beta*pow(dt1,2)*Fr*g_yg) + pow(dt1,2)*(lift3f[i]/(2.0*ds)) + (2.0*beta*yf_old1[i]) - (beta*yf_old2[i]))/(beta+(2.0*pow((dt1/ds),2)*Zeta[i])+(2.0*gamma*pow(dt1,2)/pow(ds,4)))) + (1.0-alpha_SOR_f)*yf[i];
					}
					else
					{
						xf[i]=alpha_SOR_f*(((-gamma*pow(dt1,2)/pow(ds,4))*xf[i+2] + pow(dt1,2)*((Zeta[i+1]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*xf[i+1] + pow(dt1,2)*((Zeta[i]/pow(ds,2))+(4.0*gamma/pow(ds,4)))*xf[i-1] - (gamma*pow(dt1,2)/pow(ds,4))*xf[i-2] + (beta*pow(dt1,2)*Fr*g_xg) + pow(dt1,2)*(drag3f[i]/(2.0*ds)) + (2.0*beta*xf_old1[i]) - (beta*xf_old2[i]))/(beta+(pow((dt1/ds),2)*(Zeta[i+1]+Zeta[i]))+(6.0*gamma*pow(dt1,2)/pow(ds,4)))) + (1.0-alpha_SOR_f)*xf[i];
        ...
        for (int i=0; i<Nf; i++)
        {
            ...
            {
                thetaf_global[i]=atan((yf[i]-yf[0])/(xf[i]-xf[0]));
            }
        }*/
        // to find the surface points
        // upper surface
        for (int i=0; i<Nf+1; i++)
        {
            ...
            {
                //x_sf[i]=xf[i-1] - hf[i-1]*sin(thetaf[i-1]);
				//y_sf[i]=yf[i-1] + hf[i-1]*cos(thetaf[i-1]);
				if ((xf[i]-xf[i-1])>=0.0 && thetaf[i-1]>=0.0)
				{
					x_sf[i]=xf[i-1] - hf[i-1]*sin(thetaf[i-1]);
					y_sf[i]=yf[i-1] + hf[i-1]*cos(thetaf[i-1]);
				}
				else if ((xf[i]-xf[i-1])<0.0 && thetaf[i-1]<=0.0)
				{
					x_sf[i]=xf[i-1] + hf[i-1]*sin(thetaf[i-1]);
					y_sf[i]=yf[i-1] - hf[i-1]*cos(thetaf[i-1]);
				}
				...
				{
					cout<< " Top surface end for 1 " <<endl;
				}
            }
            else
    	    {
                if (i==Nf)
				{
					if ((xf[i-1]-xf[i-2])>=0.0 && thetaf[i-1]>=0.0)
					{
						x_sf[i]=xf[i-1] - hf[i-1]*sin(thetaf[i-1]);
						y_sf[i]=yf[i-1] + hf[i-1]*cos(thetaf[i-1]);
					}
					else if ((xf[i-1]-xf[i-2])<0.0 && thetaf[i-1]<=0.0)
					{
						x_sf[i]=xf[i-1] + hf[i-1]*sin(thetaf[i-1]);
						y_sf[i]=yf[i-1] - hf[i-1]*cos(thetaf[i-1]);
					}
				...
				{
					if ((xf[i]-xf[i-2])>=0.0 && thetaf[i-1]>=0.0)
					{
						x_sf[i]=xf[i-1] - hf[i-1]*sin(thetaf[i-1]);
						y_sf[i]=yf[i-1] + hf[i-1]*cos(thetaf[i-1]);
					}
					else if ((xf[i]-xf[i-2])<0.0 && thetaf[i-1]<=0.0)
					{
						x_sf[i]=xf[i-1] + hf[i-1]*sin(thetaf[i-1]);
						y_sf[i]=yf[i-1] - hf[i-1]*cos(thetaf[i-1]);
					}
        ...
        for (int i=Nf+1; i<N_markerf; i++)
        {
            ...
            {
                //x_sf[i]=xf[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
				//y_sf[i]=yf[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				if ((xf[N_markerf-i]-xf[N_markerf-i-1])>=0.0 && thetaf[N_markerf-i-1]>=0.0)
				{
					x_sf[i]=xf[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
					y_sf[i]=yf[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				}
				else if ((xf[N_markerf-i]-xf[N_markerf-i-1])<0.0 && thetaf[N_markerf-i-1]<=0.0)
				{
					x_sf[i]=xf[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
					y_sf[i]=yf[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				}
				...
				{
					cout<< " Bottom surface end for N_markerf-1" <<endl;
				}
            }
            else
            {
				if (i==Nf+2)
				{
					if ((xf[N_markerf-i-1]-xf[N_markerf-i-2])>=0.0 && thetaf[N_markerf-i-1]>=0.0)
					{
						x_sf[i]=xf[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
						y_sf[i]=yf[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
					}
				...
				{
					if ((xf[N_markerf-i]-xf[N_markerf-i-2])>=0.0 && thetaf[N_markerf-i-1]>=0.0)
					{
						x_sf[i]=xf[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
						y_sf[i]=yf[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
					}
					else if ((xf[N_markerf-i]-xf[N_markerf-i-2])<0.0 && thetaf[N_markerf-i-1]<=0.0)
					{
						x_sf[i]=xf[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
						y_sf[i]=yf[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
					}
					...
					{
						cout<< " Bottom surface middle " <<endl;
					}
				}
            }
        }
        ...
        for (int i=0; i<Nf+1; i++)
        {
            if (i==0)
            {
				...
				if ((xfc[i+1]-xfc[i])>=0.0 && thetafc[i]>=0.0)
				{
					x_sfc[i]=xfc[i] - hf[i]*sin(thetafc[i]);
					y_sfc[i]=yfc[i] + hf[i]*cos(thetafc[i]);
				}
				else if ((xfc[i+1]-xfc[i])<0.0 && thetafc[i]<=0.0)
				{
					x_sfc[i]=xfc[i] + hf[i]*sin(thetafc[i]);
					y_sfc[i]=yfc[i] - hf[i]*cos(thetafc[i]);
				}
        ...
        for (int i=Nf+1; i<N_markerf; i++)
        {
            if (i==N_markerf-1)
            {
                //x_sfc[i]=xfc[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetaf[N_markerf-i-1]);
				//y_sfc[i]=yfc[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetaf[N_markerf-i-1]);
				if ((xfc[N_markerf-i]-xfc[N_markerf-i-1])>=0.0 && thetafc[N_markerf-i-1]>=0.0)
				{
					x_sfc[i]=xfc[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetafc[N_markerf-i-1]);
					y_sfc[i]=yfc[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetafc[N_markerf-i-1]);
				}
				else if ((xfc[N_markerf-i]-xfc[N_markerf-i-1])<0.0 && thetafc[N_markerf-i-1]<=0.0)
				{
					x_sfc[i]=xfc[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetafc[N_markerf-i-1]);
					y_sfc[i]=yfc[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetafc[N_markerf-i-1]);
				}
            ...
            else
            {
				if (i==Nf+1)
				{
					if ((xfc[N_markerf-i-1]-xfc[N_markerf-i-2])>=0.0 && thetafc[N_markerf-i-1]>=0.0)
					{
						x_sfc[i]=xfc[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetafc[N_markerf-i-1]);
						y_sfc[i]=yfc[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetafc[N_markerf-i-1]);
					}
					else if ((xfc[N_markerf-i-1]-xfc[N_markerf-i-2])<0.0 && thetafc[N_markerf-i-1]<=0.0)
					{
						x_sfc[i]=xfc[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetafc[N_markerf-i-1]);
						y_sfc[i]=yfc[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetafc[N_markerf-i-1]);
					}
				...
				{
					if ((xfc[N_markerf-i]-xfc[N_markerf-i-2])>=0.0 && thetafc[N_markerf-i-1]>=0.0)
					{
						x_sfc[i]=xfc[N_markerf-i-1] + hf[N_markerf-i-1]*sin(thetafc[N_markerf-i-1]);
						y_sfc[i]=yfc[N_markerf-i-1] - hf[N_markerf-i-1]*cos(thetafc[N_markerf-i-1]);
					}
					else if ((xfc[N_markerf-i]-xfc[N_markerf-i-2])<0.0 && thetafc[N_markerf-i-1]<=0.0)
					{
						x_sfc[i]=xfc[N_markerf-i-1] - hf[N_markerf-i-1]*sin(thetafc[N_markerf-i-1]);
						y_sfc[i]=yfc[N_markerf-i-1] + hf[N_markerf-i-1]*cos(thetafc[N_markerf-i-1]);
					}
...
#pragma acc data copy(n1, n2) copy(flag_u, flag_tu, flag_v, flag_tv, flag1, flag) copy(x_s, y_s, x_u, y_u, x_v, y_v) copyin(x1, x) copyin(y1, y)
{
        ...
        for(int i=1; i<n_u-1; i++)
        {
            for (int j=1; j<m_u-1; j++)
            {
                ...
                C1 = (dt/(2.0*Re*(x1[i][j+1]-x1[i][j])))*(((u[i][j+1]-u[i][j])/(x[i][j+1]-x[i][j]))-((u[i][j]-u[i][j-1])/(x[i][j]-x[i][j-1])))+(dt/(2*Re*(y[i][j]-y[i-1][j])))*(((u[i+1][j]-u[i][j])/(y1[i+1][j]-y1[i][j]))-((u[i][j]-u[i-1][j])/(y1[i][j]-y1[i-1][j])));
        ...
        for(int i=1; i<n_v-1; i++)
        {
            for (int j=1; j<m_v-1; j++)
            {
                ...
                C1 = (dt/(2.0*Re*(x[i][j]-x[i][j-1])))*(((v[i][j+1]-v[i][j])/(x1[i][j+1]-x1[i][j]))-((v[i][j]-v[i][j-1])/(x1[i][j]-x1[i][j-1])))+(dt/(2*Re*(y1[i+1][j]-y1[i][j])))*(((v[i+1][j]-v[i][j])/(y[i+1][j]-y[i][j]))-((v[i][j]-v[i-1][j])/(y[i][j]-y[i-1][j])));
...
#pragma acc data copy(u_red, u_black, u_red_old, u_black_old, uA_old,v_red, v_black, v_red_old, v_black_old, vA_old) copy(u,v, pressure) copyin(a_u, b_u, c_u, d_u, e_u, Bf_u, f_u,x1, x, x_s) copyin(a_v, b_v, c_v, d_v, e_v, Bf_v, f_v,y1, y, y_s)  copyin(flag_u, flag_tu, flag_v, flag_tv) copyin(u_c,v_c) copy(/*u_red, u_black, u_red_old, u_black_old, uA_old,v_red, v_black, v_red_old, v_black_old, vA_old,*/ f_p, p_c, p_c_red, p_c_red_old, p_c_black, p_c_black_old, pA_old) copyin(a_p, b_p, c_p, d_p, e_p)
{
	...
	for(int i=1; i<n-1; i++)
	{
		...
		else
		{
			...
			f_p[i][j] = -(1/dt)*((((u[i][j]-u[i][j-1])/(x[i][j]-x[i][j-1]))+((v[i][j]-v[i-1][j])/(y[i][j]-y[i-1][j]))) - (((flag_tu[i][j]*(u[i][j]-u_body))/(x[i][j]-x[i][j-1]))+(-(flag_tu[i][j-1]*(u[i][j-1]-u_body))/(x[i][j]-x[i][j-1]))+((flag_tv[i][j]*(v[i][j]-v_body))/(y[i][j]-y[i-1][j]))+(-(flag_tv[i-1][j]*(v[i-1][j]-v_body))/(y[i][j]-y[i-1][j]))));
                }
            }
        }
        ...
        if (file_lift3f.is_open())
	{
	    file_lift3f << fixed << setprecision(6) << time1 << " " ;
            for(int i=0; i<Nf; i++)
	    {
                file_lift3f << lift3f[i] << " " ;
	    }
	    file_lift3f<< " " <<endl;
            //file_lift3f.close();
	}
	if (file_drag3f.is_open())
	{
	    file_drag3f << fixed << setprecision(6) << time1 << " " ;
            for(int i=0; i<Nf; i++)
	    {
                file_drag3f << drag3f[i] << " " ;
	    }
	    file_drag3f<< " " <<endl;
            //file_drag3f.close();
	}
